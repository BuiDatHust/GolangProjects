...
func isBackendAlive(u *url.URL) bool {
	...
	if err != nil {
		log.Printf("Site unreachebale, error:", err)
		return false
	}
	defer conn.Close()
	return true
...
func main() {
	...
	var port int
	flag.StringVar(&serverList, "backends", "", "Load balenced backends, use commas to sepearate")
	...
	for _, tok := range tokens {
		...
		proxy.ErrorHandler = func(writer http.ResponseWriter, request *http.Request, e error) {
			...
			attemps := GetAttempsFromContext(request)
			log.Printf("%s(%s) Attemping retry %d", request.RemoteAddr, request.URL.Path, attemps)
	...
	log.Printf("Load balencer started at: %d\n", port)
